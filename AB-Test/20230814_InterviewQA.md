...
## 对于稀疏指标（罕见事件、坏样本数等），该用哪种检验方法？
对于稀疏指标（例如坏样本数、罕见事件发生次数等），通常不适用于t检验，因为t检验假设数据符合正态分布，而稀疏指标往往不满足这个假设。相反，Mann-Whitney U检验是一种非参数检验方法，不需要对数据做分布假设，因此在这种情况下更适用。
总结如下：
当你拥有两个组的连续变量数据，且这些数据满足正态分布的假设时，可以选择使用t检验。
当你拥有两个组的稀疏指标数据，或者数据不满足正态分布假设时，可以选择使用Mann-Whitney U检验。
除了Mann-Whitney U检验，还有其他非参数检验方法，例如Wilcoxon秩和检验和Kruskal-Wallis检验等，可以根据具体情况选择适合的方法。
需要注意的是，在选择统计方法时，除了数据的分布特征外，还应考虑样本量、变量类型等因素。此外，检验结果只能作为统计推断的一部分，需要综合考虑实际背景和业务含义来做出判断。
